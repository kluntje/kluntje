...
test('should add Element to Array, if its not already in it and return resulting array', () => {
  ...
  expect(newArray).toEqual(['newElement']);
  expect(testArray).toEqual(['newElement']);

  newArray = pushIfNew(testArray, 'anotherElement');

  expect(newArray).toEqual(['newElement', 'anotherElement']);
  expect(testArray).toEqual(['newElement', 'anotherElement']);
});
